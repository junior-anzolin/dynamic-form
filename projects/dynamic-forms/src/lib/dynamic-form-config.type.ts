import { ConfigOption, ExtensionOption, ManipulatorOption, TypeOption, ValidationMessageOption, ValidatorOption, WrapperOption } from "@ngx-formly/core/lib/services/formly.config";
...
export interface DynamicFormConfig {
    ...
    export class DynamicFormConfigProvider {
        ...
        toFormlyConfig(mode: 'view' | 'edit'): ConfigOption {
            return {
                types: this.dynamicFormsConfig.types?.map((itemProvider: DynamicFormType) => new DynamicFormTypeProvider(itemProvider)).map((type) => type.toTypeOption(mode)),
                wrappers: this.dynamicFormsConfig.wrappers?.map((itemProvider: DynamicFormWrappersType) => new DynamicFormWrapperProvider(itemProvider)).map((wrapper) => wrapper.toWrapperOption(mode)),
...
export class DynamicFormTypeProvider {
    ...
    toTypeOption(mode: 'view' | 'edit'): TypeOption {
        if(Object.prototype.hasOwnProperty.call(this.dynamicFormsConfig, "unique")) {
            return (this.dynamicFormsConfig as DynamicFormTypeUnique).unique; 
        }
        let type = (this.dynamicFormsConfig as DynamicFormTypeMultiple);
...
export type DynamicFormWrappersType = DynamicFormWrappersUnique | DynamicFormWrappersMultiple;
...
export class DynamicFormWrapperProvider {
    ...
    toWrapperOption(mode: 'view' | 'edit'): WrapperOption {
        if(Object.prototype.hasOwnProperty.call(this.dynamicFormsConfigWrapper, "unique")) {
            return (this.dynamicFormsConfigWrapper as DynamicFormWrappersUnique).unique; 
        }
        let type = (this.dynamicFormsConfigWrapper as DynamicFormWrappersMultiple);
        return mode == 'view' ? type.view : type.edit;
    }
}